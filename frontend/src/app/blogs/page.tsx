...
const BlogPage: React.FC = () => {
  ...
  useEffect(() => {
    const getArticles = async () => {
      ...
      // Extract unique tags
      const tags = Array.from(new Set(fetchedArticles.flatMap(article => article.tags ? [article.tags.name] : [])));
      setUniqueTags(tags);
    };
    getArticles();
  }, []);
  ...
  useEffect(() => {
    const results = articles.filter(article => 
      ...
      (selectedTag === '' || (article.tags && article.tags.name === selectedTag))
    );
    ...
    <section className="py-16 bg-white font-inter">
      <div className="bg-blue-50 py-12 mb-10 px-4">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-bold text-center p-3 text-blue-500 mb-8">Blogs</h1>
                ...
                <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-end p-4">
                  ...
                  {post.tags && (
                    <p className="text-white text-sm mt-2">
                      <span className="font-semibold">Tags:</span> {post.tags.name}
                    </p>
                  )}
                </div>
